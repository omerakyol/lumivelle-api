...
public class RedisCacheManager : ICacheManager
{
    ...
    public RedisCacheManager(IConfiguration configuration)
    {
        ...
        if (!string.IsNullOrEmpty(cacheConfig.Password)) configurationOptions.Password = cacheConfig.Password;
    ...
    public T Get<T>(string key)
    {
        ...
        RedisInvoker(x =>
        {
            ...
            if (data.HasValue) result = JsonSerializer.DeserializeFromString<T>(data);
        });
        return result;
    }
    ...
    public object Get(string key)
    {
        ...
        {
            var data = _cache.StringGet(key);
            if (data.HasValue) result = JsonSerializer.DeserializeFromString<object>(data);
        });
        return result;
    }
    ...
    {
        await _redis.KeyDeleteByPatternAsync(pattern, 0);
    }
